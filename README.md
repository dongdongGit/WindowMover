...
- **开机自启** - 可选开机自动启动功能（通过 Windows 任务计划程序实现，支持管理员权限启动）
- **管理员启动** - 可选以管理员身份运行，勾选后自动 UAC 提权重启
- **静默启动** - 可选最小化启动，启动时隐藏主窗口
- **功能开关** - 可随时禁用鼠标中键移动功能
- **强制置顶** - 移动窗口后自动将其置顶，确保窗口可见
- **DPI 感知** - 支持高 DPI 显示器，界面清晰锐利
- **自动更新** - 启动时自动检查 GitHub 最新版本，支持在线下载并自动安装更新
...
- **开机自动启动** - 程序随系统启动自动运行（通过任务计划程序实现，兼容管理员模式）
- **总是最小化启动** - 启动时隐藏主窗口，直接运行在后台
- **以管理员身份启动** - 以管理员权限运行程序，勾选时弹出 UAC 提示并自动重启
- **启动时自动检查更新** - 自动检测 GitHub 最新版本，可一键下载安装
...
### 最新更新
- 新增自动检查更新功能，支持在线下载并自动安装更新（通过 GitHub Releases API）
- 系统托盘右键菜单新增「检查更新」项
- 新增「以管理员身份启动」选项，支持 UAC 提权
- 开机自启动改用 Windows 任务计划程序（schtasks），修复管理员模式下自启动不生效的问题
- 自动清理旧版注册表启动项
